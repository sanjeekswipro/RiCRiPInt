 ...
 *
 * $Id: arenavm.c,v 1.89.1.1.1.1 2013/12/19 11:27:09 anon Exp $
 * $HopeName: MMsrc!arenavm.c(EBDSDK_P.1) $
...
SRCID(arenavm, "$Id: arenavm.c,v 1.89.1.1.1.1 2013/12/19 11:27:09 anon Exp $");
...
static Bool VMArenaCheck(VMArena vmArena)
{
  ...
  CHECKL(vmArena->reserveLimit > 0);
  CHECKL(vmArena->reserveLimit >= VMArenaReserved(arena)
         /* .reservelimit.overflow: The arena can breach the reserve
          * limit by a little because of overheads in the VM module.
          * Pick 144 kB as the design limit. */
         || VMArenaReserved(arena) - vmArena->reserveLimit <= 144*1024);
 ...
 * size, amount of virtual address space that should be reserved.
 * minSize, minimum address space that is acceptable to the caller.
 */
static Res vmChunkCreate(Chunk *chunkReturn, VMArena vmArena,
                         Size size, Size minSize)
{
  ...
  /* Try reserving a block of VM of the required size; if that doesn't
   * work, try smaller sizes down to minSize. */
  res = VMCreate(&vm, size);
  if (res == ResRESOURCE) {
    Count ctr = VMArenaTryCOUNT;
    Size tryInterval = (size - minSize) / VMArenaTryCOUNT;

    do {
      --ctr;
      res = VMCreate(&vm, minSize + ctr * tryInterval);
    } while (res == ResRESOURCE && ctr > 0);
  }
  if (res != ResOK)
    goto failVMCreate;
...
failChunkInit:
  vmArenaUnmap(vmArena, vm, base, chunkStructLimit);
failChunkMap:
failChunkAlloc:
failBootInit:
  VMDestroy(vm);
...
static Res VMArenaInit(Arena *arenaReturn, ArenaClass class, va_list args)
{
  ...
  vmArena->blacklist = ZoneSetAdd(arena,
                                  ZoneSetAdd(arena, ZoneSetEMPTY,
                                             (Addr)(Word)1),
                                  (Addr)(Word)-1);
  ...
  if (class == VMArenaClassGet()) {
    vmArena->reserveLimit = SizeMAX;
    res = vmChunkCreate(&chunk, vmArena, userSize, userSize);
  } else {
    vmArena->reserveLimit = userSize;
    res = vmChunkCreate(&chunk, vmArena, userSize, 1);
  }
  if (res != ResOK)
...
static void VMArenaFinish(Arena arena, Bool abort)
{
  VMArena vmArena;
  Ring node, next;
  VM arenaVM;
  ...
  RING_FOR(node, &arena->chunkRing, next) {
    ...
    VMChunk vmChunk = Chunk2VMChunk(chunk);
    Index base, start, end;

    if (abort) {
      /* Find all the allocated bits and unmap. */
      base = chunk->allocBase;
      do {
        if (!BTFindLongSetRange(&start, &end, chunk->allocTable,
                                base, chunk->pages, 1))
          break;
        vmArenaUnmap(vmArena, vmChunk->vm,
                     PageIndexBase(chunk, start),
                     PageIndexBase(chunk, end));
        BTResRange(chunk->allocTable, start, end);
        base = end;
      } while (base < chunk->pages);
      /* Bypassed the spare pages mechanism, so fake it. */
      BTSetRange(vmChunk->noSparePages, 0, chunk->pageTablePages);
      /* Unmap the page table. */
      base = 0;
      do {
        if (!BTFindLongSetRange(&start, &end, vmChunk->pageTableMapped,
                                base, chunk->pageTablePages, 1))
          break;
        vmArenaUnmap(vmArena, vmChunk->vm,
                     TablePageIndexBase(chunk, start),
                     TablePageIndexBase(chunk, end));
        BTResRange(vmChunk->pageTableMapped, start, end);
        base = end;
      } while (base < chunk->pageTablePages);
    }
    vmChunkDestroy(chunk);
  }
 ...
 */
Size VMArenaReserved(Arena arena)
{
  Size reserved;
  Ring node, next;
 ...
 */
void tablePagesUnmapUnused(VMChunk vmChunk,
                           Addr tablePageBase, Addr tablePageLimit)
{
  Chunk chunk;
  Addr cursor;
...
{
  Chunk newChunk;
  Size neededChunkSize;
  Res res;

  if (!pagesFindFreeWithSegPref(baseIndexReturn, chunkReturn,
                                vmArena, pref, size, FALSE)) {
    /* try and extend, but don't worry if we can't */
    neededChunkSize = size
                      + ChunkOverheadEstimate(VMArena2Arena(vmArena), size);
    res = vmChunkCreate(&newChunk, vmArena,
                        MAX(vmArena->extendBy, neededChunkSize),
                        neededChunkSize);
    if (res == MPS_RES_OK)
      /* Verify usable space is enough. */
      AVER(AddrOffset(PageIndexBase(newChunk, newChunk->allocBase),
                      newChunk->limit)
           >= size);
                           ...
                           VMArena vmArena, SegPref pref, Size size)
{
  ...
  if (pagesFindFreeInZones(baseIndexReturn, chunkReturn, vmArena, size,
    ...
    return ResOK;
  /* try and extend */
  neededChunkSize = size + ChunkOverheadEstimate(VMArena2Arena(vmArena), size);
  /* Check it's not already over the limit (see .reservelimit.overflow) */
  if (vmArena->reserveLimit <= VMArenaReserved(VMArena2Arena(vmArena)))
    return ResRESOURCE;
  allowed = vmArena->reserveLimit - VMArenaReserved(VMArena2Arena(vmArena));
  if (allowed < neededChunkSize)
    return ResRESOURCE;
  res = vmChunkCreate(&newChunk, vmArena,
                      MIN(MAX(vmArena->extendBy, neededChunkSize), allowed),
                      neededChunkSize);
  if (res != ResOK) {
    vmArena->extendBy = MIN(vmArena->extendBy, neededChunkSize);
    return res;
  } else {
    /* Next time, try a chunk of the same size. */
    vmArena->extendBy = AddrOffset(newChunk->base, newChunk->limit);
  }
  /* Verify usable space is enough. */
  AVER(AddrOffset(newChunk->allocBase, newChunk->limit) >= size);
...
/* VMArenaExtend -- extends the address space reserved */
static Res VMArenaExtend(Arena arena, Addr base, Size size)
{
  VMArena vmArena;
  ...
  while ( VMArenaReserved(arena) < vmArena->reserveLimit ) {
    Chunk newChunk;
    Size allowed = vmArena->reserveLimit - VMArenaReserved(arena);
    Res res;

    res = vmChunkCreate(&newChunk, vmArena,
                        MIN(vmArena->extendBy, allowed), 1);
    if (res != ResOK)
      return res;
    /* Next time, try a chunk of the same size. */
    vmArena->extendBy = AddrOffset(newChunk->base, newChunk->limit);
...
mps_arena_class_t MPS_CALL mps_arena_class_vm(void)
{
...
mps_arena_class_t MPS_CALL mps_arena_class_vmnz(void)
{
