 ...
 *
 * $Id: tract.c,v 1.13.11.1.1.1 2013/12/19 11:27:10 anon Exp $
 * $HopeName: MMsrc!tract.c(EBDSDK_P.1) $
...
SRCID(tract, "$Id: tract.c,v 1.13.11.1.1.1 2013/12/19 11:27:10 anon Exp $");
...
Bool ChunkCheck(Chunk chunk)
{
  ...
  CHECKL((Addr)&chunk->pageTable[chunk->pageTablePages]
         <= PageIndexBase(chunk, chunk->allocBase));
  /* check there's enough space in the page table for itself and the rest */
  /* INDEX_OF_ADDR returns unsigned, so it'll underflow to large values. */
  CHECKL(INDEX_OF_ADDR(chunk, (Addr)chunk->pageTable) < chunk->pages);
...
/* ChunkOverheadEstimate - estimate the size of the chunk overhead
 *
 * This estimates the size of the chunk tables for the given usable
 * space in the chunk.  Trying to keep this exactly in step with the
 * allocation code is complex (since the code is) and misguided (since
 * it works in the other direction, taking the chunk size and splitting
 * off pieces for the tables), so we don't.  Instead the callers should
 * check if they got enough space. */

Size ChunkOverheadEstimate(Arena arena, Size usableSize)
{
  Size pageSize = arena->alignment;
  Count usablePages = usableSize / pageSize + 1;
  double multiplier; /* relative increase from adding the page table */

  /* .estimate.overflow: We know these calculations won't overflow,
   * because PageStructs are much smaller than pages. */

  /* Account for page table, and more table to describe the table pages,
   * and more to describe those pages, etc.
   *   Sum(n, 0, inf, (sizeof(PageStruct)/pageSize)^n) */
  multiplier = (double)pageSize / (pageSize - sizeof(PageStruct));
  return (SizeAlignUp((multiplier * usablePages
                       * (sizeof(PageStruct)
                          + 1.0/MPS_WORD_WIDTH /* one bit for alloc table */)),
                      pageSize)
          + 3 * pageSize /* some constant overhead */);
}
...
Bool TractOfAddr(Tract *tractReturn, Arena arena, Addr addr)
{
  Bool b;
  Index i;
  Chunk chunk = NULL /* pacify compiler */;
...
{
  Tract tract = NULL /* pacify compiler */;
  Bool found;
