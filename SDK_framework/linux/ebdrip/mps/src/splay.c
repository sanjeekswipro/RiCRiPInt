 ...
 *
 * $Id: splay.c,v 1.17.4.1.1.1 2013/12/19 11:27:07 anon Exp $
 * $HopeName: MMsrc!splay.c(EBDSDK_P.1) $
...
SRCID(splay, "$Id: splay.c,v 1.17.4.1.1.1 2013/12/19 11:27:07 anon Exp $");
...
Compare SplayLess(void *key, SplayNode node)
{
  UNUSED(key); UNUSED(node);
  return CompareLESS;
}

SplayNode SplayTreeFirst(SplayTree tree) {
  SplayNode node;
  AVERT(SplayTree, tree);

  if (SplayTreeRoot(tree) == NULL) {
    node = NULL;
  } else if (SplaySplay(&node, tree, NULL, SplayLess)) {
    NOTREACHED;
  } else {
    AVER(SplayNodeLeftChild(node) == NULL);
  }
...
static Compare SplayFindFirstCompare(void *key, SplayNode node) 
{
  ...
  if (SplayNodeLeftChild(node) != NULL &&
     (*testTree)(tree, SplayNodeLeftChild(node), closureP)) {
    return CompareLESS;
  } else if ((*testNode)(tree, node, closureP)) {
    return CompareEQUAL;
  } else {
    AVER(SplayNodeRightChild(node) != NULL &&
         (*testTree)(tree, SplayNodeRightChild(node), closureP));
                           ...
                           void *closureP)
{
  ...
  if (node == NULL || !(*testTree)(tree, node, closureP))
