 ...
 *
 * $Id: poolamc.c,v 1.67.10.1.1.1 2013/12/19 11:27:06 anon Exp $
 * $HopeName: MMsrc!poolamc.c(EBDSDK_P.1) $
...
SRCID(poolamc, "$Id: poolamc.c,v 1.67.10.1.1.1 2013/12/19 11:27:06 anon Exp $");
...
static Bool amcNailboardCheck(amcNailboard board)
{
  ...
  CHECKL((Align)1 << board->markShift == PoolAlignment(amcGenPool(board->gen)));
  /* weak check for BTs @@@@ */
...
static Res AMCWInit(Pool pool, va_list arg)
{
  return amcInitComm(pool, RankSetSingle(RankWEAK), arg);
}
...
Res AMCFix(Pool pool, ScanState ss, Seg seg, Ref *refIO)
{
  ...
  Bool shieldUp = FALSE;/* whether we have exposed seg */
...
/* AMCResolve -- resolve a reference to the pool
 *
 * See design.mps.poolamc.fix.
 */
Bool AMCResolve(Pool pool, ScanState ss, Seg seg, Ref *refIO)
{
  Arena arena;
  AMC amc;
  Bool wasRetained;
  Format format;        /* cache of pool->format */
  Ref ref;              /* reference to be resolved */
  Ref newRef;           /* new location, if moved */
  ...
  newRef = (*format->isMoved)(ref);
  if (newRef == (Addr)0) {
    /* If object isn't nailed for any of these traces, it is dead from
     * their point of view (the scan method should clear the reference
     * or change its mind and call MPS_RETAIN). */
    if (TraceSetInter(ss->traces, SegNailed(seg))
        /* @@@@ Probably need to keep track of nailing for each trace to
         * get a precise result for multiple traces. */
        && (!amcSegHasNailboard(seg) || amcNailGetMark(seg, ref))) {
      wasRetained = TRUE;
      goto returnRes;
    }
    ...
    /* reference to broken heart (which will be snapped out) */
    wasRetained = TRUE;
    STATISTIC_STAT(++ss->snapCount);
    *refIO = newRef;
  }
...
static void AMCWalk(Pool pool, Seg seg, FormattedObjectsStepMethod f, void *p)
{
  ...
  AVERT(Seg, seg);
  AVER(FUNCHECK(f));
  /* p is arbitrary so can't be checked */
...
static void amcWalkAll(Pool pool, FormattedObjectsStepMethod f, void *p)
{
  Arena arena;
  Ring ring, next, node;
...
/* AMCZNoGrey - disabled grey method */

void AMCZNoGrey(Pool pool, Trace trace, Seg seg)
{
  AVERT(Pool, pool);
  AVERT(Trace, trace);
  AVERT(Seg, seg);
...
/* AMCZNoScan - disabled scan method */

Res AMCZNoScan(Bool *totalReturn, ScanState ss, Pool pool, Seg seg)
{
  AVER(totalReturn != NULL);
  AVERT(ScanState, ss);
  AVERT(Pool, pool);
...
/* AMCWPoolClass -- the class definition */

DEFINE_POOL_CLASS(AMCWPoolClass, this)
{
...
DEFINE_POOL_CLASS(AMCZPoolClass, this)
{
  ...
  this->init = AMCZInit;
  this->grey = AMCZNoGrey;
  this->scan = AMCZNoScan;
}
  

/* mps_class_amc -- return the pool class descriptor to the client */

mps_class_t MPS_CALL mps_class_amc(void)
{
  return (mps_class_t)AMCPoolClassGet();
}

/* mps_class_amcw -- return the pool class descriptor to the client */

mps_class_t MPS_CALL mps_class_amcw(void)
{
  return (mps_class_t)AMCWPoolClassGet();
}

/* mps_class_amcz -- return the pool class descriptor to the client */

mps_class_t MPS_CALL mps_class_amcz(void)
{
...
typedef struct mps_amc_apply_closure_s {
  void (MPS_CALL *f)(mps_addr_t object, void *p, size_t s);
  void *p;
  size_t s;
} mps_amc_apply_closure_s;

static void mps_amc_apply_iter(Addr addr, Format format, Pool pool, void *p)
{
...
void MPS_CALL mps_amc_apply(mps_pool_t mps_pool,
                   void (MPS_CALL *f)(mps_addr_t object, void *p, size_t s),
                   void *p, size_t s)
{
  ...
  amcWalkAll(pool, mps_amc_apply_iter, &closure_s);
