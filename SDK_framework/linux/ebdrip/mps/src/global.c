 ...
 *
 * $Id: global.c,v 1.28.11.1.1.1 2013/12/19 11:27:10 anon Exp $
 * $HopeName: MMsrc!global.c(EBDSDK_P.1) $
...
SRCID(global, "$Id: global.c,v 1.28.11.1.1.1 2013/12/19 11:27:10 anon Exp $");
...
/* GlobalsStaticReset -- reset global state to initial state */

void GlobalsStaticReset(void)
{
  arenaRingInit = FALSE;
}
...
failLockAlloc:
  ControlFree(arena, arena->enabledMessageTypes, BTSize(MessageTypeLIMIT));
failMessageAlloc:
  return res;
}
...
void GlobalsFinish(Globals arenaGlobals, Bool abort)
{
  Arena arena;
  Rank rank;

  if (!abort) AVERT(Globals, arenaGlobals);
  arena = GlobalsArena(arenaGlobals);
 ...
 *
 * This is like the initial finalizations in a Destroy method, except
 * there's no separate GlobalsDestroy.  */

void GlobalsPrepareToDestroy(Globals arenaGlobals, Bool abort)
{
  Arena arena;
  ...
  arenaGlobals->lock = NULL; /* pretend no locking to make globals check OK */
 ...
 * To save time, actually polls only at intervals of ArenaPollALLOCTIME.
 * However, if tryGC is true, GC is polled (TracePoll) regardless.
 * 
 * This is where time is "stolen" from the mutator in addition to doing
 * what it asks and servicing accesses. */

#ifdef MPS_PROD_EPCORE
void (ArenaPoll)(Globals globals, Bool tryGC)
{
...
void ArenaPoll(Globals globals, Bool tryGC)
{
  ...
  if (!tryGC && (globals->insidePoll || size < globals->pollThreshold))
    return;
