 ...
 *
 * $Id: poolams.c,v 1.79.1.1.1.1 2013/12/19 11:27:10 anon Exp $
 * $HopeName: MMsrc!poolams.c(EBDSDK_P.1) $
...
SRCID(poolams, "$Id: poolams.c,v 1.79.1.1.1.1 2013/12/19 11:27:10 anon Exp $");
...
static void amsSegFreeCheck(AMSSeg amsseg)
{
  Pool pool;
                      ...
                      Bool reservoirPermit, va_list args)
{
  ...
  /* Splat first, because we'll check on failure */
  ...
  RingAppend((AMSClassOf(ams)->allocRing)(ams, SegRankSet(seg), size),
             &amsseg->segRing);

  amsseg->sig = AMSSegSig;
  ams->free += amsseg->grains;
  AVERT(AMSSeg, amsseg);
...
static void AMSSegFinish(Seg seg)
{
  ...
  DebugPoolFreeSplat(AMS2Pool(ams), SegBase(seg), SegLimit(seg));
  ...
  AVER(ams->free >= amsseg->free);
  ams->free -= amsseg->free;
...
#define SPLIT_TABLES(table, setHighRangeFn) \
  ...
  RingAppend((AMSClassOf(ams)->allocRing)(ams, SegRankSet(segHi),
                                          SegSize(segHi)),
             &amssegHi->segRing);
                        ...
                        Bool withReservoirPermit)
{
  ...
  res = AMSClassOf(ams)->segSize(&prefSize, pool, size, rankSet);
  if (res != ResOK)
    goto failSize;

  res = SegAlloc(&seg, (AMSClassOf(ams)->segClass)(), segPref, prefSize, 
  ...
  if (res != ResOK) { /* try to allocate one that's just large enough */
    ...
    if (minSize == prefSize)
      goto failSeg;
    res = SegAlloc(&seg, (AMSClassOf(ams)->segClass)(), segPref, minSize, 
                   pool, withReservoirPermit);
    if (res != ResOK)
      goto failSeg;
  }
                         ...
                         Bool withReservoirPermit)
{
  ...
  ring = (AMSClassOf(ams)->allocRing)(ams, rankSet, size);
...
static void amsRangeCondemn(Seg seg, Index base, Index limit)
{
  if (base != limit) {
    AVER(base < limit);
    AVER(limit <= Seg2AMSSeg(seg)->grains);
      ...
      if (amsseg->ambiguousFixes) {
        ...
        while(j < amsseg->grains
          ...
          if ( format->skip != NULL )
            ...
            next = AddrAdd(p, alignment);
          AVER_CRITICAL(AddrIsAligned(next, alignment));
...
/* AMSResolve -- the pool class resolve method */

static Bool AMSResolve(Pool pool, ScanState ss, Seg seg, Ref *refIO)
{
  AMSSeg amsseg;
  Index i;                      /* the index of the fixed grain */
  Ref ref;
...
static void AMSReclaim(Pool pool, Trace trace, Seg seg)
{
  ...
  if (debug != NULL) {
    ...
    while(j < grains && AMS_FIND_WHITE_RANGE(&i, &j, seg, j, grains)) {
      AVER(!AMS_IS_INVALID_COLOUR(seg, i));
      /* This wouldn't need to splat blocks that are already free, but
       * it can't always tell, so it doesn't bother. */
...
/* AMSFreeSize -- return the size of free blocks in the pool */
...
DEFINE_CLASS(AMSPoolClass, this)
{
  ...
  INHERIT_CLASS(acpc, AbstractCollectPoolClass);
  PoolClassMixInFormat(acpc);
  acpc->name = "AMS";
  acpc->size = sizeof(AMSStruct);
  acpc->offset = offsetof(AMSStruct, poolStruct);
  acpc->init = AMSInit;
  acpc->finish = AMSFinish;
  acpc->bufferClass = RankBufClassGet;
  acpc->bufferFill = AMSBufferFill;
  acpc->bufferEmpty = AMSBufferEmpty;
  acpc->whiten = AMSCondemn;
  acpc->blacken = AMSBlacken;
  acpc->scan = AMSScan;
  acpc->fix = AMSFix;
  acpc->fixEmergency = AMSFix;
  acpc->resolve = AMSResolve;
  acpc->reclaim = AMSReclaim;
  acpc->freewalk = AMSFreeWalk;
  acpc->freeSize = AMSFreeSize;
  acpc->describe = AMSDescribe;
  this->segSize = AMSSegSizePolicy;
  this->allocRing = AMSPoolRing;
  this->segClass = AMSSegClassGet;
}
...
DEFINE_ALIAS_CLASS(AMSDebugPoolClass, AMSPoolClass, this)
{
  AbstractCollectPoolClass acpc = &this->acpClass;
  ...
  acpc->name = "AMSDBG";
  acpc->size = sizeof(AMSDebugStruct);
  acpc->debugMixin = AMSDebugMixin;
}
