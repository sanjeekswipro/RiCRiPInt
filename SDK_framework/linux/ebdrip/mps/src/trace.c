 ...
 *
 * $Id: trace.c,v 1.117.1.1.1.1 2013/12/19 11:27:09 anon Exp $
 * $HopeName: MMsrc!trace.c(EBDSDK_P.1) $
...
SRCID(trace, "$Id: trace.c,v 1.117.1.1.1.1 2013/12/19 11:27:09 anon Exp $");
...
Bool ScanStateCheck(ScanState ss)
{
  ...
  if (ss->seg != NULL) CHECKD(Seg, ss->seg);
...
void ScanStateInit(ScanState ss, TraceSet ts, Arena arena,
                   Rank rank, Seg seg, ZoneSet white)
{
  TraceId ti;
  Trace trace;
...
static Res traceScanRootRes(TraceSet ts, Rank rank, Arena arena, Root root)
{
  ...
  ScanStateInit(&ss, ts, arena, rank, NULL, white);
...
/* traceFlip -- blacken the mutator, flipping the roots of the next rank */
...
static Res rootFlip(Root root, void *p)
{
  ...
  if (TraceSetInter(RootGrey(root), rf->ts) != TraceSetEMPTY) {
    AVER(RootRank(root) >= rf->rank);
...
static void traceFlip(Trace trace)
{
  ...
  AVERT(Trace, trace);
  arena = trace->arena;
  AVER(trace->state == TraceUNFLIPPED || trace->state == TraceFLIPPING);
  AVER(!TraceSetIsMember(arena->flippedTraces, trace)
       || trace->state == TraceFLIPPING);

  EVENT_PP(TraceFlipBegin, trace, arena);

  if (trace->state == TraceUNFLIPPED) {
    ShieldSuspend(arena);
    trace->state = TraceFLIPPING;

    /* .flip.alloc: Allocation needs to become black now. While we flip at the
     * start, we can get away with always allocating black. This needs to change
     * when we flip later (i.e. have a read-barrier collector), so that we
     * allocate grey or white before the flip and black afterwards. For
     * instance, see design.mps.poolams.invariant.alloc. */
  ...
  if (trace->mayMove != ZoneSetEMPTY && trace->rootRank == RankEXACT)
    LDAge(arena, trace->mayMove);

  rfc.ts = TraceSetSingle(trace);
  rfc.arena = arena; rfc.rank = trace->rootRank;
  rfc.moreGrey = FALSE;
  /* At the moment we must scan all roots in one go, because we don't have a
   * mechanism for shielding them (not even protectable ones). */
  res = RootsIterate(ArenaGlobals(arena), rootFlip, &rfc);
  AVER(res == ResOK);
 ...
 *
 * Returns ResLIMITATION if there aren't any available trace IDs.
 *
...
Res TraceCreate(Trace *traceReturn, Arena arena)
{
  ...
  return ResLIMITATION; /* no trace IDs available */
...
static Res traceScanSegRes(TraceSet ts, Rank rank, Arena arena, Seg seg)
{
  ...
  if (ZoneSetInter(white, SegSummary(seg)) == ZoneSetEMPTY) {
    ...
    ScanStateStruct ss;
    ScanStateInit(&ss, ts, arena, rank, seg, white);
    ...
    /* If the client didn't raise an error, take the fix method's return code. */
    if (res == ResOK)
      res = ss.res;
...
void TraceFix(ScanState ss, Ref *refIO)
{
  Ref ref;
  Tract tract;
  Pool pool;
      ...
      STATISTIC_STAT
        ({
          ...
          if (TRACT_SEG(&seg, tract)) {
            ++ss->segRefCount;
            EVENT_P(TraceFixSeg, seg);
            UNUSED(seg); /* hack: unused in non-event varieties */
          }
        });
    }
  } else {
    /* This assert means GC found a dangling pointer! */
    AVER(ss->rank < RankEXACT /* See design.mps.trace.exact.legal */
...
void TraceFixEmergency(ScanState ss, Ref *refIO)
{
  Ref ref;
  Tract tract;
  Pool pool;
...
/* TraceResolve -- resolve a reference */

Bool TraceResolve(ScanState ss, Ref *refIO)
{
  Ref ref;
  Tract tract;
  Bool res = TRUE;
  ...
  if (tract) {
    if (TraceSetInter(TractWhite(tract), ss->traces) != TraceSetEMPTY) {
      ...
      if (TRACT_SEG(&seg, tract)) {
        ...
        STATISTIC(++ss->whiteSegRefCount);
        EVENT_P(TraceFixSeg, seg);
        res = pool->class->resolve(pool, ss, seg, refIO);
      }
    } else { 
      /* Tract isn't white. Don't compute seg for non-statistical */
      /* variety. See design.mps.trace.fix.tractofaddr */
  ...
  EVENT_PPAUU(TraceResolve, ss, refIO, ref, ss->rank, res);

  /* If it was updated, record it in the summary. */
  if (res)
    ss->fixedSummary = RefSetAdd(ss->arena, ss->fixedSummary, *refIO);
  return res;
}
...
void TraceStart(Trace trace, double mortality, double finishingTime)
{
  ...
  trace->state = TraceUNFLIPPED; trace->rootRank = RankAMBIG;
}
...
static void traceQuantum(Trace trace)
{
  ...
  do {
    switch(trace->state) {
    case TraceUNFLIPPED:
      /* all traces flip at the start at the moment */
      /* fall-through */
    case TraceFLIPPING: case TraceFLIPPED: {
      Arena arena = trace->arena;
      Seg seg = NULL /* pacify compiler */;
      Rank segRank = RankLIMIT;
      Bool gotSeg;

      gotSeg = traceFindGrey(&seg, &segRank, arena, trace->ti);
      if (trace->rootRank < RankLIMIT && segRank >= trace->rootRank)
        traceFlip(trace);
      else if (gotSeg)
        traceScanSeg(TraceSetSingle(trace), segRank, arena, seg);
      else
    ...
    default:
      ...
      break;
           ...
           /* Can't stop while flipping, because the roots aren't protected. */
           && (trace->emergency || trace->state == TraceFLIPPING
...
/* TracePerform -- run a given trace to the end */

void TracePerform(Trace trace)
{
  AVERT(Trace, trace);
