 ...
 *
 * $Id: bt.c,v 1.33.11.1.1.1 2013/12/19 11:27:10 anon Exp $
 * $HopeName: MMsrc!bt.c(EBDSDK_P.1) $
...
SRCID(bt, "$Id: bt.c,v 1.33.11.1.1.1 2013/12/19 11:27:10 anon Exp $");
...
/* BTFindSetRange -- find a set range of bits in a bit table
 *
 * Starts searching at the low end of the search range.
 *
 * Like BTFindResRange, mutatis mutandis.  */

static Bool BTFindSetRange(Index *baseReturn, Index *limitReturn,
                           BT bt,
                           Index searchBase, Index searchLimit,
                           Count minLength, Count maxLength)
{
  Bool foundSet;         /* true if a set bit is found */
  Index setBase;         /* base of a candidate set range */
  Index unseenBase;      /* base of testing so far */
  Index minLimit;        /* limit of minimal acceptable range */
  Index setLimit;        /* limit of search for a candidate range */
  ...
  foundSet = FALSE;     /* don't know first set bit */
  minLimit = 0;         /* avoid spurious compiler warning */
  setBase = searchBase; /* haven't seen anything yet */
  unseenBase = searchBase;  /* haven't seen anything yet */
  setLimit = searchLimit - minLength + 1;

  while (setBase < setLimit) {
    Index resIndex;  /* index of last reset bit found */
    Bool foundRes = FALSE; /* true if a reset bit is found */

    /* Find the first set bit if it's not already known */
    if (!foundSet) {
      BTFindSet(&foundSet, &setBase, bt, unseenBase, setLimit);
      if (!foundSet) {
        /* failure */
        return FALSE;
      }
      unseenBase = setBase + 1;
      minLimit = setBase + minLength;
    }

    /* Look to see if there is any reset bit in the minimum range */
    BTFindResHigh(&foundRes, &resIndex, bt, unseenBase, minLimit);
    if (!foundRes) {
      /* Found minimum range. Extend it. */
      Index resBase;   /* base of search for reset bit */
      Index resLimit;  /* limit search for reset bit */
      foundRes = FALSE;
      resBase = minLimit;
      resLimit = setBase + maxLength;
      if (resLimit > searchLimit)
      ...
      AVER(resIndex - setBase >= minLength);
      AVER(resIndex - setBase <= maxLength);
      *baseReturn = setBase;
      *limitReturn = resIndex;
      return TRUE;
...
/* BTFindLongSetRange -- find long range of set bits in a bit table
 *
 * Like BTFindLongResRange, mutatis mutandis.  */

Bool BTFindLongSetRange(Index *baseReturn, Index *limitReturn,
                        BT bt,
                        Index searchBase, Index searchLimit,
                        Count length)
{
  /* All parameters are checked by BTFindSetRange. */
  return BTFindSetRange(baseReturn, limitReturn,
                        bt,
                        searchBase, searchLimit,
                        length, searchLimit - searchBase);
}
