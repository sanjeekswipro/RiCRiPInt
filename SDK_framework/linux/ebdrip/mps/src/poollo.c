 ...
 *
 * $Id: poollo.c,v 1.31.10.1.1.1 2013/12/19 11:27:07 anon Exp $
 * $HopeName: MMsrc!poollo.c(EBDSDK_P.1) $
...
SRCID(poollo, "$Id: poollo.c,v 1.31.10.1.1.1 2013/12/19 11:27:07 anon Exp $");
                   ...
                   FormattedObjectsStepMethod f, void *p)
{
  ...
  AVERT(Seg, seg);
  AVER(FUNCHECK(f));
  /* p is arbitrary and can't be checked */
                        ...
                        Size size, Bool withReservoirPermit)
{
  ...
  Ring node, nextNode;
  LO lo;
  LOSeg loseg = NULL /* pacify compiler */;
  Arena arena;
  Addr base = NULL /* pacify compiler */, limit = NULL /* pacify compiler */;
...
/* LOResolve -- resolve a reference */

static Bool LOResolve(Pool pool, ScanState ss, Seg seg, Ref *refIO)
{
  LO lo;
  LOSeg loseg;
  Ref ref;
  ...
  i = AddrOffset(SegBase(seg), (Addr)ref) >> lo->alignShift;
  return BTGet(loseg->mark, i);
}
...
static void LOGrey(Pool pool, Trace trace, Seg seg)
{
  AVERT(Pool, pool);
  AVERT(Trace, trace);
  AVERT(Seg, seg);
...
static void LOBlacken(Pool pool, TraceSet traceSet, Seg seg)
{
  AVERT(Pool, pool);
...
static Res LOScan(Bool *totalReturn, ScanState ss, Pool pool, Seg seg)
{
...
DEFINE_POOL_CLASS(LOPoolClass, this)
{
  ...
  this->grey = LOGrey;
  this->blacken = LOBlacken;
  this->scan = LOScan;
  this->fix = LOFix;
  this->fixEmergency = LOFix;
  this->resolve = LOResolve;
...
mps_class_t MPS_CALL mps_class_lo(void)
{
