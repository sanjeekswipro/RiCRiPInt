 ...
 *
 * $Id: pool.c,v 1.85.1.1.1.1 2013/12/19 11:27:04 anon Exp $
 * $HopeName: MMsrc!pool.c(EBDSDK_P.1) $
...
SRCID(pool, "$Id: pool.c,v 1.85.1.1.1.1 2013/12/19 11:27:04 anon Exp $");
...
Bool PoolCheck(Pool pool)
{
  ...
  CHECKL(SizeIsAligned(pool->managedSize, ArenaAlign(pool->arena)));
...
/* PoolClear -- clear all allocs from pool */
...
void PoolClear(Pool pool)
{
  ...
  /* Should this do something with SACs? */
  (*pool->class->clear)(pool);
  EVENT_P(PoolClear, pool);
}
              ...
              Bool withReservoirPermit, DebugInfo info)
{
  ...
  res = (*pool->class->alloc)(pReturn, pool, size, withReservoirPermit, info);
  if (res != ResOK)
    return res;
  ...
  if (info != NULL)
    EVENT_PAWWW(PoolAllocDebug, pool, *pReturn, size,
                info->location, info->class);
  else
...
void PoolWalk(Pool pool, Seg seg, FormattedObjectsStepMethod f, void *p)
{
  AVERT(Pool, pool);
  AVERT(Seg, seg);
  AVER(FUNCHECK(f));
  /* p is an arbitrary value, hence can't be checked. */

  (*pool->class->walk)(pool, seg, f, p);
}
...
/* PoolManagedSize -- return the size of memory managed by this pool */

Size PoolManagedSize(Pool pool)
{
  AVERT(Pool, pool);

  return pool->managedSize;
}


/* PoolFreeSize -- return the size of free blocks in this pool */
...
/* PoolHasAddr - test whether the address is managed by the pool */

Bool PoolHasAddr(Pool pool, Addr addr)
{
  Pool addrPool = NULL /* pacify compiler */;
  Arena arena;
  Bool managed;
