 ...
 *
 * $Id: mpm.h,v 1.173.1.1.1.1 2013/12/19 11:27:08 anon Exp $
 * $HopeName: MMsrc!mpm.h(EBDSDK_P.1) $
...
#define size_tAlignUp(s, a) (((s) + (a) - 1) & ~((size_t)(a) - 1))
...
#define ResCheck(res) ((res) >= ResOK && (res) < ResLIMIT)
...
extern Size PoolManagedSize(Pool pool);
...
extern void PoolClear(Pool pool);
...
extern void PoolWalk(Pool pool, Seg seg, FormattedObjectsStepMethod f, void *v);
...
extern void TraceFix(ScanState ss, Ref *refIO);
extern void TraceFixEmergency(ScanState ss, Ref *refIO);
extern Bool TraceResolve(ScanState ss, Ref *refIO);
...
/* TRACE_SCAN_BEGIN -- equivalent to impl.h.mps MPS_SCAN_BEGIN */
...
/* TRACE_FIX1 -- update summary and check whether in a white zone
 *
...
/* TRACE_RETAIN2 -- retain (use after TRACE_FIX1) */

#define TRACE_RETAIN2(ss, refIO) \
  ((*(ss)->fix)(ss, refIO))

/* TRACE_RETAIN -- equivalent to impl.h.mps MPS_RETAIN */

#define TRACE_RETAIN(ss, refIO, cond) \
  BEGIN \
    if (TRACE_FIX1(ss, *(refIO)) && (cond)) TRACE_RETAIN2(ss, refIO); \
  END

/* TRACE_SCAN_END -- equivalent to impl.h.mps MPS_SCAN_END */
...
extern void TracePerform(Trace trace);
...
extern void ArenaDestroy(Arena arena, Bool abort);
extern Res ArenaInit(Arena arena, ArenaClass class);
extern void ArenaFinish(Arena arena, Bool abort);
...
extern void GlobalsFinish(Globals arena, Bool abort);
extern Res GlobalsCompleteCreate(Globals arenaGlobals);
extern void GlobalsPrepareToDestroy(Globals arenaGlobals, Bool abort);
extern void GlobalsStaticReset(void);
...
extern void (ArenaPoll)(Globals globals, Bool tryGC);
#ifdef MPS_PROD_EPCORE
#define ArenaPoll(globals, tryGC) BEGIN DISCARD(globals); DISCARD(tryGC); END
#endif
/* .nogc.why: The Harlequin RIP doesn't use incremental GC, so */
...
extern Size ArenaCommittedMax(Arena arena);
...
extern Bool (SegIsShielded)(Seg seg);
...
#define ShieldRaise(arena, seg, mode) \
  BEGIN DISCARD(arena); DISCARD(seg); DISCARD(mode); END
#define ShieldLower(arena, seg, mode) \
  BEGIN DISCARD(arena); DISCARD(seg); DISCARD(mode); END
#define SegIsShielded(seg) ((void)sizeof(seg), FALSE)
#define ShieldEnter(arena) DISCARD(arena)
#define ShieldLeave(arena) DISCARD(arena)
#define ShieldExpose(arena, seg) BEGIN DISCARD(arena); DISCARD(seg); END
#define ShieldCover(arena, seg) BEGIN DISCARD(arena); DISCARD(seg); END
#define ShieldSuspend(arena) DISCARD(arena)
#define ShieldResume(arena) DISCARD(arena)
#define ShieldFlush(arena) DISCARD(arena)
#endif
...
extern void RootGreyAdd(Root root, Trace trace);
extern TraceSet RootGrey(Root root);
...
#if defined(STACKPROBE)

extern void StackProbe(Size depth);

#elif defined(STACKPROBE_NONE)

#define StackProbe(depth) DISCARD(depth)

#else

#error "No stack probing configured."
...
#define STATISTIC(gather) BEGIN (void)(gather); END
