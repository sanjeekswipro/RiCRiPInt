 ...
 *
 * $Id: poolmvff.c,v 1.32.1.1.1.1 2013/12/19 11:27:05 anon Exp $
 * $HopeName: MMsrc!poolmvff.c(EBDSDK_P.1) $
...
SRCID(poolmvff, "$Id: poolmvff.c,v 1.32.1.1.1.1 2013/12/19 11:27:05 anon Exp $");
...
static void MVFFAddToFreeList(Addr *baseIO, Addr *limitIO, MVFF mvff) {
  ...
  AVER(res == ResOK); /* failure here is an overlapping free */
...
/* MVFFClear -- clear all allocs from pool */

void MVFFClear(Pool pool)
{
  MVFF mvff;
  Seg seg;
  Ring ring, node, nextNode;
...
/* MVFFFreeSize -- total free bytes */

Size MVFFFreeSize(Pool pool)
{
  MVFF mvff;
...
mps_class_t MPS_CALL mps_class_mvff(void)
{
  return (mps_class_t)(MVFFPoolClassGet());
}

mps_class_t MPS_CALL mps_class_mvff_debug(void)
{
...
static Bool MVFFCheck(MVFF mvff)
{
  ...
  CHECKL(PoolManagedSize(MVFF2Pool(mvff)) >= mvff->free);
