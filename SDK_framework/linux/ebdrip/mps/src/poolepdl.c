 ...
 *
 * $Id: poolepdl.c,v 1.49.1.1.1.1 2013/12/19 11:27:06 anon Exp $
 * $HopeName: MMsrc!poolepdl.c(EBDSDK_P.1) $
...
SRCID(poolepdl, "$Id: poolepdl.c,v 1.49.1.1.1.1 2013/12/19 11:27:06 anon Exp $");
...
#define DEBUG_MAX(x, n) \
  STATISTIC((void)(epdl->debug_##x < (n) && (epdl->debug_##x = (n), TRUE)))
...
typedef struct EPDLStruct {     /* EPDL pool outer structure */
  ...
  EPDLBlock lastFree;           /* point of last free */
                        ...
                        Addr base, Addr limit, Size size)
{
  ...
  res = PoolAlloc(&new, epdl->blockPool, sizeof(EPDLBlockStruct),
                  /* withReservoirPermit */ FALSE, NULL);
  if (res != ResOK) {
                                      ...
                                      Addr limit)
{
  ...
  if (base == block->base) {
    if (limit == block->limit) { /* remove whole block */
      ...
      /* find the pointer to this block */
      ...
      *blockPtr = block->next;
      if (epdl->lastFree == block)
        epdl->lastFree = *blockPtr;
    ...
    if (limit != block->limit) { /* split the block */
      EPDLBlock new = NULL /* pacify compiler */;
      Res res;

      res = EPDLNewBlock(&new, epdl, limit, block->limit,
...
static void EPDLBlockFreeSegs(EPDL epdl, EPDLBlock block)
{
  ...
  if (block->limit >= segLimit) { /* then we have some segs to free */
    ...
    while(block->limit > segLimit) { /* free a seg and move on */
      ...
      DebugPoolFreeCheck(pool, SegBase(seg), SegLimit(seg));
      DEBUG_INC(segFree);
...
{
  Res res;
  EPDLBlock prev, next, block = NULL /* pacify compiler */;
  Size size;
  ...
  if ((next != NULL) && (next->limit <= base)) {
    ...
    if (epdl->lastFree != NULL && epdl->lastFree->limit <= base)
      /* after lastFree */
      next = epdl->lastFree;
...
static Res EPDLAddSeg(EPDL epdl, Size size, Bool withReservoirPermit)
{
  ...
  DEBUG_MAX(maxTotal, PoolManagedSize(pool));

  DebugPoolFreeSplat(pool, SegBase(seg), SegLimit(seg));

  res = EPDLAddToFreeList(&block, epdl, SegBase(seg), SegLimit(seg));
  UNUSED(block);
  return res;
}
                     ...
                     Bool withReservoirPermit, DebugInfo info)
{
  ...
  Addr new;
  EPDLBlock block;
  EPDLBlock *blockPtr = NULL /* pacify compiler */;
...
static void EPDLFinish(Pool pool)
{
  ...
  RING_FOR(node, ring, nextNode) {
    ...
    AVER(SegPool(seg) == pool);
    /* It's too expensive to check free space now, because some parts of these */
    /* segments can be allocated, so it'd have to map over the free list. */
    SegFree(seg);
  }
...
/* EPDLFreeSize -- total free bytes */

Size EPDLFreeSize(Pool pool)
{
  EPDL epdl;
...
mps_class_t MPS_CALL mps_class_epdl(void)
{
  return (mps_class_t)(EnsureEPDLPoolClass());
}

mps_class_t MPS_CALL mps_class_epdr(void)
{
  return (mps_class_t)(EnsureEPDRPoolClass());
}

mps_class_t MPS_CALL mps_class_epdl_debug(void)
{
  return (mps_class_t)(EnsureEPDLDebugPoolClass());
}

mps_class_t MPS_CALL mps_class_epdr_debug(void)
{
...
static Bool EPDLCheck(EPDL epdl)
{
  ...
  CHECKL(PoolManagedSize(EPDLPool(epdl)) >= epdl->free + epdl->lost);
  ...
  CHECKL(epdl->lastFree == NULL || EPDLBlockCheck(epdl->lastFree));
  return TRUE;
}
