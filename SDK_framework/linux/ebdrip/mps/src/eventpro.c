 ...
 *
 * $HopeName: MMsrc!eventpro.c(EBDSDK_P.1) $
 */
...
{
  return event->any.codeAndClock & 0xFF;
}
...
static Res eventStringCopy(EventString *str_o, EventString str)
{
  ...
  newStr->len = str->len;
  (void)memcpy(&(newStr->str), &(str->str), str->len);
  *str_o = newStr;
  return ResOK;
}
...
typedef struct symbolStruct {
  Word id; /* Also used as table key - may not need storing. */
  EventString name;
} symbolStruct;
...
typedef struct labelStruct {
  Word id;
  mps_clock_t time;
  Addr addr; /* Also used as table key - may not need storing */
} labelStruct;
...
/* TextLabel -- return intern id for given text (or 0 if none) */
static void TextLabelFind(Word key, void *value, void *data)
{
  Symbol entry = value ;
  Symbol search = data ;
  UNUSED(key) ;
  if ( search->id == 0 ) {
    if ( search->name->len == entry->name->len &&
         strncmp(search->name->str, entry->name->str, entry->name->len) == 0 )
      *search = *entry ;
  }
}

Word TextLabel(EventProc proc, EventString name)
{
  symbolStruct search ;
  search.id = 0 ;
  search.name = name ;
  TableMap(proc->internTable, TextLabelFind, &search) ;
  return search.id ;
}
...
Res EventRead(EventCode *codeReturn, mps_clock_t *currentTimeIO,
              Event *eventReturn, EventProc proc)
{
  ...
  res = proc->reader(proc->readerP, &codeAndClock, sizeof(codeAndClock));
  if (res != ResOK) return res;

  code = codeAndClock & 0xFF;
  ...
  event->any.codeAndClock = codeAndClock;
  restOfEvent = PointerAdd(event, sizeof(codeAndClock));
  if (code == EventCodeIntern) { /* the only string event */
    /* read enough to get the length */
    res = proc->reader(proc->readerP, restOfEvent,
                       internStrOffset - sizeof(codeAndClock));
    if (res != ResOK) return res;
    /* read the rest */
    ...
    res = proc->reader(proc->readerP, restOfEvent,
                       length - sizeof(codeAndClock));
    if (res != ResOK) return res;
  }

  /* codeAndClock >> 8 are the new low bits of the clock. */
  newTime = (*currentTimeIO & ~(~(mps_clock_t)0 >> 8)) | (codeAndClock >> 8);
  /* If codeAndClock wrapped round, do the carry. */
  if (newTime < *currentTimeIO)
    newTime += (~(mps_clock_t)0 >> 8) + 1;

  *codeReturn = code; *currentTimeIO = newTime; *eventReturn = event;
  return ResOK;
}
...
Res EventRecord(EventProc proc, Event event, mps_clock_t etime)
{
  Res res;

  switch(EventGetCode(event)) {
  case EventCodeIntern: {       /* id, label */
  ...
  case EventCodeArenaDestroy: /* arena */
  case EventCodePoolFinish: { /* pool */
    /* Remove label for it, if any. Ignore error if there wasn't one. */
    (void)TableRemove(proc->labelTable, (Word)event->p.p0);
    res = ResOK;
  } break;
  default:
    res = ResOK;
                    ...
                    EventProcReader reader, void *readerP)
{
  ...
  assert(CHECKFIELD(EventUnion, any.codeAndClock, EventWSStruct, codeAndClock));
...
static void deallocItem(Word key, void *value, void *data)
{
  UNUSED(key);
  UNUSED(data);
  free(value);
...
static void deallocSym(Word key, void *value, void *data)
{
  UNUSED(key);
  UNUSED(data);
