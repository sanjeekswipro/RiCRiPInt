/* impl.c.seg: SEGMENTS
 *
 * $Id: seg.c,v 1.35.1.1.1.1 2013/12/19 11:27:07 anon Exp $
 * $HopeName: MMsrc!seg.c(EBDSDK_P.1) $
...
SRCID(seg, "$Id: seg.c,v 1.35.1.1.1.1 2013/12/19 11:27:07 anon Exp $");
...
static Res segTrivDescribe(Seg seg, mps_lib_FILE *stream)
{
  ...
  if (RankSetIsMember(seg->rankSet, RankWEAKFINAL)) {
     res = WriteF(stream, " weak-final", NULL);
     if (res != ResOK) return res;
  }
  res = WriteF(stream, "\n",
               "  white  $B\n", (WriteFB)seg->white,
