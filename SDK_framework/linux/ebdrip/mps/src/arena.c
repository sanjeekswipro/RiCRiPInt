 ...
 *
 * $Id: arena.c,v 1.91.1.1.1.1 2013/12/19 11:27:09 anon Exp $
 * $HopeName: MMsrc!arena.c(EBDSDK_P.1) $
...
SRCID(arena, "$Id: arena.c,v 1.91.1.1.1.1 2013/12/19 11:27:09 anon Exp $");
...
failReservoirInit:
  GlobalsFinish(ArenaGlobals(arena), FALSE);
failGlobalsInit:
  return res;
}
...
void ArenaFinish(Arena arena, Bool abort)
{
  if (!abort)
    ReservoirFinish(ArenaReservoir(arena));
  arena->sig = SigInvalid;
  GlobalsFinish(ArenaGlobals(arena), abort);
  if (!abort) {
    LocusFinish(arena);
...
void ArenaDestroy(Arena arena, Bool abort)
{
  AVERT(Arena, arena);

  GlobalsPrepareToDestroy(ArenaGlobals(arena), abort);
  ...
  (*arena->class->finish)(arena, abort);
               ...
               Bool withReservoirPermit)
{
  ...
  if (res == ResOK) {
    goto goodAlloc;
  } else if (withReservoirPermit && ResIsAllocFailure(res)) {
    Res resRes = ReservoirWithdraw(&base, &baseTract, reservoir, size, pool);
    if (resRes == ResOK)
      goto goodAlloc;
    AVER(resRes == ResMEMORY);
  }
...
void ArenaFree(Addr base, Size size, Pool pool)
{
  ...
  EVENT_PAWP(ArenaFree, arena, base, size, pool);
  return;
}
...
Size ArenaCommittedMax(Arena arena)
{
  AVERT(Arena, arena);
  return arena->committedMax;
}
