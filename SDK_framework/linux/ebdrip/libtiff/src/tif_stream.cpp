/* $HopeName: HQNlibtiff!src:tif_stream.cpp(EBDSDK_P.1) $ */

/* $Id: src:tif_stream.cpp,v 1.3.2.1.1.1 2013/12/19 11:24:39 anon Exp $ */
...
_tiffisReadProc(thandle_t fd, tdata_t buf, tsize_t size)
{
        ...
        return (tsize_t)data->myIS->gcount();
}
...
{
	tiffis_data	*data = (tiffis_data *)fd;
	int		pos = (int)data->myIS->tellg();
	int		len;

	data->myIS->seekg(0, ios::end);
	len = (int)data->myIS->tellg();
	data->myIS->seekg(pos);
...
_tiffStreamOpen(const char* name, const char* mode, void *fd)
{
	...
	if( strchr(mode, 'w') ) {
		...
		data->myStreamStartPos = (long)data->myOS->tellp();
		...
		data->myStreamStartPos = (long)data->myIS->tellg();
		// Open for reading.
		tif = TIFFClientOpen(name, mode,
				(thandle_t) data,
