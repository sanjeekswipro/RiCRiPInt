 ...
 * \ingroup hdphoto
 *
 * $HopeName: COREwmphoto!export:windowsmediaphoto.h(EBDSDK_P.1) $ 
 *
 * Copyright (C) Microsoft Corporation. All rights reserved.
 *
 * \brief
 * Header with image structure etc
 */
...
#include <limits.h>
#include <core.h>
#define malloc(a) wmp_malloc(a)
#define calloc(a,b) wmp_calloc(a,b)
#define free(a) wmp_free(a)
...
/* GG additions */
/* Check I8, I16, I32, U8, U16, U32 sizes correspond to
 * GG int8, int16 int32, uint8, uint16, uint32 sizes.
 */
#if ( SCHAR_MAX != 0x7F || SHRT_MAX != 0x7FFF || INT_MAX != 0x7FFFFFFF )
#error "Mismatch between Microsoft type sizes and GG type sizes"
#endif

#if ( UCHAR_MAX != 0xFF || USHRT_MAX != 0xFFFF || UINT_MAX != 0xFFFFFFFF )
#error "Mismatch between Microsoft type sizes and GG type sizes"
#endif
/* GG End */
...
#define Report(err, szExp, szFile, nLine) \
    HQTRACE(TRUE, ("FAILED: %ld=%s" CRLF, (err), (szExp))); \
    HQTRACE(TRUE, ("        %s:%ld" CRLF, (szFile), (nLine))); \
