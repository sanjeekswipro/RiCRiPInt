 ...
 * \ingroup scanconvert
 *
 * $HopeName: CORErender!src:scadobe.c(EBDSDK_P.1) $
 *
 * Copyright (C) 2003-2010 Global Graphics Software Ltd. All rights reserved.
 * Global Graphics Software Ltd. Confidential Information.
 *
 * \brief
 * Device resolution Adobe pixel-touching scan conversion for fills.
 *
 ...
 * minor variant of the Adobe "any area covered" pixel touching rule. The
 * variation is that we fill a pixel if there is any area to the right of a
 * left contour, or any area to the left of a right contour. The tests for
 * the area covered is optimised by performing a half-step the first time a
 * line is encountered (moving the current point from the centre of the span
 * intersection with the scanline to the transition point between two
 * scanlines), full steps for the intermediate scanlines, and another half
 * step when the line terminates.
 */
...
static inline void one_step_set(NBRESS *thread, dda_info_t *span)
{
  ...
  step_to_transition(thread, denom, &x, &xe, &ox, &oxe) ;

  thread->u1.ncx = x ;
  thread->xe = xe ;
  thread->nmindy -= 2 ;

  /* Any area covered by the pixel means it should be included. */
  if ( thread->si < 0 ) { /* Negative gradient, so x < ox */
    span->left = x ;
    if ( oxe == 0 ) /* There was no area in pixel ox; we just grazed it. */
      ox -= 1 ;
    span->right = ox ;
  } else { /* Vertical or positive gradient, so x >= ox */
    span->left = ox ;
    if ( xe == 0 ) /* There was no area in pixel x; we just grazed it. */
      x -= 1 ;
    span->right = x ;
  }
}
...
static inline void one_step_extend(NBRESS *thread, dda_info_t *span)
{
  ...
  /* Any area covered by the pixel means it should be included. */
  if ( thread->si < 0 ) { /* Negative gradient, so x < ox */
    INLINE_MIN32(span->left, x, span->left) ;
    if ( oxe == 0 ) /* There was no area in pixel ox; we just grazed it. */
      ox -= 1 ;
    INLINE_MAX32(span->right, span->right, ox) ;
  } else { /* Vertical or positive gradient, so x >= ox */
    INLINE_MIN32(span->left, ox, span->left) ;
    if ( xe == 0 ) /* There was no area in pixel x; we just grazed it. */
      x -= 1 ;
    INLINE_MAX32(span->right, span->right, x) ;
  }
}
...
static inline void one_final(NBRESS *thread, dda_info_t *span)
{
  ...
  if ( denom != 0 ) {
    ...
    if ( thread->si < 0 ) { /* Negative gradient, so x < ox */
      ...
      if ( thread->xe == 0 ) /* There was no area in pixel ncx; we just grazed it. */
        span->right -= 1 ;
    }
  } else {
...
void scanconvert_adobe(render_blit_t *rb, NFILLOBJECT *nfill, int32 therule,
                       spanlist_t *mergedspans)
{
  ...
  HQASSERT(nfill->converter == SC_RULE_TOUCHING,
           "Scan conversion rule should be set for touching.") ;
