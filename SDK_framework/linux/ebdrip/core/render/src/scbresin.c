 ...
 * \ingroup scanconvert
 *
 * $HopeName: CORErender!src:scbresin.c(EBDSDK_P.1) $
 *
 * Copyright (C) 2009-2010 Global Graphics Software Ltd. All rights reserved.
 * Global Graphics Software Ltd. Confidential Information.
 *
 * \brief
 * Device resolution Bresenham pixel-touching scan conversion for
 * fills, rounding towards inside of fills.
 */
...
enum {
  BRES_THREAD_NEITHER = 0, /* Initial value of flags */
  BRES_THREAD_LEFT = 1,
  BRES_THREAD_RIGHT = 2
} ;
...
static inline void one_step_set(NBRESS *thread, dda_info_t *span)
{
  ...
  if ( !thread->ntype ) {
    ...
    /* Steep gradient line does not cover more than the pixel in which it
       originated. It uses the pixel centres rather than the edges. We round
       boundary pixels upwards, towards the interior of the shape. */
    span->left = span->right = x ;
...
static inline void one_step_extend(NBRESS *thread, dda_info_t *span)
{
  ...
  if ( !thread->ntype ) {
    ...
    /* Steep gradient line does not cover more than the pixel in which it
       originated. It uses the pixel centres rather than the edges. We round
       boundary pixels downwards, towards the interior of the shape. */
    dcoord tx = x - (xe == 0) ;
    if ( tx > span->right )
      span->right = tx ;
  ...
  thread->xe = xe ;
  thread->nmindy -= 2 ;
  thread->flags = BRES_THREAD_RIGHT ;
}
...
static inline void one_final(NBRESS *thread, dda_info_t *span)
{
  ...
  if ( denom != 0 ) {
    ...
    if ( !thread->ntype ) {
      ...
      if ( si < 0 ) {
        switch ( thread->flags ) {
        case BRES_THREAD_LEFT: /* Last used as left contour */
          if ( oxe < denom )
            ox -= 1 ;
          break ;
        case BRES_THREAD_RIGHT: /* Last used as right contour */
          if ( oxe <= denom )
            ox -= 1 ;
          break ;
        case BRES_THREAD_NEITHER:
        default:
          /** \todo ajcd 2009-01-25: This case will happen if a fill is
              clipped to just the last line of the thread. What should we do
              about it? */
          HQTRACE(TRUE, ("Thread finished in neither left nor right state")) ;
          break ;
        }
...
void scanconvert_bresin(render_blit_t *rb, NFILLOBJECT *nfill, int32 therule,
                        spanlist_t *mergedspans)
{
  ...
  HQASSERT(nfill->converter == SC_RULE_BRESENHAM,
           "Scan conversion rule should be set for Bresenham.") ;
...
/* Fill commences in a later band; make sure we update the clipping */
  ...
  if ( scanline > info.clip.y1 ) {
    /* Make sure we update the clipping for scanlines in the band preceeding
       the fill */
    if ( info.update ) {
      updatebandclip(info.update, info.surface_handle,
