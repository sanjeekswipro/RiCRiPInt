 ...
 * \ingroup cstandard
 *
 * $HopeName: HQNc-standard!export:murmurhash3.h(EBDSDK_P.1) $
 *
...
/* Block read - if the platform needs to do endian-swapping or can
...
static inline void MurmurHash3_32( const void * key, int len,
                                   uint32 seed, uint32 *out )
{
...
static inline void MurmurHash3_128( const void *key, const int len,
                                    void *seed, void *out )
{
  const uint8 * data = (const uint8*)key;
  const int nblocks = len / 16;

  uint32 h1 = (( uint32 * )seed )[0];
  uint32 h2 = (( uint32 * )seed )[1];
  uint32 h3 = (( uint32 * )seed )[2];
  uint32 h4 = (( uint32 * )seed )[3];
  ...
  tail = (const uint8*)(data + nblocks*16);
...
#else /* defined( PLATFORM_IS_64BIT ) */
    ...
    void *seed, void *out )
 {
  ...
  uint64 h1 = (( uint64 * )seed )[0];
  uint64 h2 = (( uint64 * )seed )[1];
...
#endif /* ! defined( PLATFORM_IS_64BIT ) */

#endif /* defined ( MURMURHASH3_H ) */
