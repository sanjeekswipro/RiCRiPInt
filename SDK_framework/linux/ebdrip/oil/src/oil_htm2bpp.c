 ...
 *
 * $HopeName: SWebd_OIL_example_gg!src:oil_htm2bpp.c(EBDSDK_P.1) $
 */
/*! \file
 *  \ingroup OIL
 *  \brief HTM example screening module for 2bpp
 *
 * A primitive screening module example.
 *
 * Its aim is to implement a four level threshold screen that could be used
 * as an alternative to a PostScript thresholds.
 *
 ...
 *
 * Function htm2bpp_remapHTtables() can reset these mappings as required for 
 * the specified screen mode.
 * @see htm2bpp_remapHTtables().
 */
static OIL_TyScreens st2bppScreenLookup[]=
{
  { GG_OBJ_GFX,
    { PMS_SCREEN_2BPP_GFX_CYAN,
      PMS_SCREEN_2BPP_GFX_MAGENTA,
      PMS_SCREEN_2BPP_GFX_YELLOW,
      PMS_SCREEN_2BPP_GFX_BLACK } },
  { GG_OBJ_IMAGE,
    { PMS_SCREEN_2BPP_IMAGE_CYAN,
      PMS_SCREEN_2BPP_IMAGE_MAGENTA,
      PMS_SCREEN_2BPP_IMAGE_YELLOW,
      PMS_SCREEN_2BPP_IMAGE_BLACK } },
  { GG_OBJ_TEXT,
    { PMS_SCREEN_2BPP_TEXT_CYAN,
      PMS_SCREEN_2BPP_TEXT_MAGENTA,
      PMS_SCREEN_2BPP_TEXT_YELLOW,
      PMS_SCREEN_2BPP_TEXT_BLACK } }
};


#define OIL_EBD_SCRN_OBJECTS_2BPP (sizeof(st2bppScreenLookup) / sizeof(st2bppScreenLookup[0]))

extern HqBool RIPCALL do2bppHalftone(sw_htm_instance *instance,
 ...
 */
HTI* g_2bppHTI[4] = { &g_HTI_0, &g_HTI_1, &g_HTI_2, &g_HTI_3 };


static void htm2bpp_setupHTtables();
 ...
 *
 * We only have to worry, therefore, about a single halftone instance in any
 * one call.
 *
 ...
 *   Default is @a false.
 *
 * This example is so simple that we can declare our instance structures
 * statically, so all that's really involved here is validating our
 * parameters, initializing the inverse screen cells if needed, determine
 * our appropriate halftone instance, increment its reference count, and
 * fill in the details for the RIP.
 *
 * In the case of the bit depth of the source raster, we can handle both
 * 8 and 16 bits, so if the RIP hasn't already decided upon a bit depth,
 * we opt for 8 bits on the premise that the extra depth won't benefit our
 * very primitive screens, and 8 bits will RIP faster than 16.
 *
...
{ int i ;
  ...
  HQASSERT(i < 4, "Failed to find instance") ;
}
                                          ...
                                          const sw_htm_render_info *render_info)
{
  ...
  if ( render_info->dst_bit_depth != 2 )
...
sw_htm_api *htm2bpp_getInstance()
{
  ...
  htmApi.info.name = (uint8*)"htm2bpp" ;
  htmApi.info.display_name = (uint8*)"htm2bpp example screening module" ;
  ...
  htmApi.DoHalftone = do2bppHalftone ;
 ...
 * 
 * st2bppScreenLookup array must be initialized before this function can be called.
 */
static void htm2bpp_setupHTtables()
{
  ...
  /* setup g_2bppHTI */
  for(obj = 0; obj < OIL_EBD_SCRN_OBJECTS_2BPP; obj++)
  {
    for(color = OIL_Cyan; color <= OIL_Black; color++)
    {
      if(!GetScreenInfoFromPMS(st2bppScreenLookup[obj].eHtTables[color] , &pScreenInfo))
      {
        HQFAILV(("ebd_scrn_open_file: failed to get screen info for object=%d, colorant=%d", obj, color));
          return;
      }
      g_2bppHTI[color]->pHTables[obj].uWidth = (unsigned short)pScreenInfo->nCellWidth;
      g_2bppHTI[color]->pHTables[obj].uHeight = (unsigned short)pScreenInfo->nCellHeight;
      g_2bppHTI[color]->pHTables[obj].ditherMatrix = &pScreenInfo->pTable;
    }
  }
}
 ...
 *
 * This function configures the \c st2bppScreenLookup structure with appropriate values so that 
 * subsequent calls to htm2bpp_setupHTtables() will fetch the correct tables from the PMS.
 */
void htm2bpp_remapHTtables(OIL_eScreenQuality eImageQuality, OIL_eScreenQuality eGraphicsQuality, OIL_eScreenQuality eTextQuality)
{
  ...
  case OIL_Scrn_LowLPI:
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_IMAGE_CYAN;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_IMAGE_MAGENTA;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_IMAGE_YELLOW;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_IMAGE_BLACK;
    break;
  case OIL_Scrn_MediumLPI:
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_GFX_CYAN;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_GFX_MAGENTA;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_GFX_YELLOW;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_GFX_BLACK;
    break;
  case OIL_Scrn_HighLPI:
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_TEXT_CYAN;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_TEXT_MAGENTA;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_TEXT_YELLOW;
    st2bppScreenLookup[GG_OBJ_IMAGE].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_TEXT_BLACK;
    break;
  }
  ...
  {
  case OIL_Scrn_LowLPI:
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_IMAGE_CYAN;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_IMAGE_MAGENTA;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_IMAGE_YELLOW;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_IMAGE_BLACK;
    break;
  case OIL_Scrn_MediumLPI:
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_GFX_CYAN;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_GFX_MAGENTA;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_GFX_YELLOW;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_GFX_BLACK;
    break;
  case OIL_Scrn_HighLPI:
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_TEXT_CYAN;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_TEXT_MAGENTA;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_TEXT_YELLOW;
    st2bppScreenLookup[GG_OBJ_GFX].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_TEXT_BLACK;
    break;
  }
  ...
  {
  case OIL_Scrn_LowLPI:
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_IMAGE_CYAN;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_IMAGE_MAGENTA;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_IMAGE_YELLOW;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_IMAGE_BLACK;
    break;
  case OIL_Scrn_MediumLPI:
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_GFX_CYAN;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_GFX_MAGENTA;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_GFX_YELLOW;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_GFX_BLACK;
    break;
  case OIL_Scrn_HighLPI:
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Cyan] = PMS_SCREEN_2BPP_TEXT_CYAN;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Magenta] = PMS_SCREEN_2BPP_TEXT_MAGENTA;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Yellow] = PMS_SCREEN_2BPP_TEXT_YELLOW;
    st2bppScreenLookup[GG_OBJ_TEXT].eHtTables[OIL_Black] = PMS_SCREEN_2BPP_TEXT_BLACK;
    break;
  }
