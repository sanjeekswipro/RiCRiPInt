...
/* Moved the function (and macro) "scale()" to the top of the file because it
   is called by all 3 versions of MicroType code (FCO1, FCO2 and FCO3). Duplicate
   versions of this function have been removed from FC_INTFL.C and FC_INTF2.C.
 */
#if defined (_WIN32_WCE)
#define scale(value, pixel, gray) ( gray \
	? ((((((UL32) value) << pixel->inBinPlaces)  \
		 + (SL32)pixel->inPrecHaPix) / (SL32)pixel->inPrecPixel) \
		 << pixel->ouBinPlaces) \
	: (((((UL32) value) << (pixel->inBinPlaces + pixel->ouBinPlaces))  \
         + (SL32)pixel->inPrecHaPix) / (SL32)pixel->inPrecPixel) \
		 )
#else            

#if defined( ANSI_DEFS )
GLOBAL SL32 scale (SL32 value, PIXEL_DATA *pixel, BOOLEAN gray)
#else
GLOBAL SL32 scale( value, pixel, gray)
SL32 value;
PIXEL_DATA* pixel;
BOOLEAN gray;
#endif
...
/* 
	To prevent an integer overflow condition from happening, MicroType graymaps will be scaled
	slightly differently than bitmaps.
	
	For MicroType bitmaps, "value" will continue to be scaled by (inBinPlaces + ouBinPlaces)
	before rounding	and converting to pixels.
	
	For MicroType graymaps, "value" will now be scaled by inBinpaces before rounding and converting
	to pixels. The pixel result will then be scaled by ouBinPlaces.
	
	Note: This is the method used in the function in_VectorScaleGrid().
*/

if (gray)
	result =   ((((((UL32) value) << pixel->inBinPlaces)
			 + (SL32)pixel->inPrecHaPix) / (SL32)pixel->inPrecPixel)
			 << pixel->ouBinPlaces);
else
	result =   ((((UL32) value) << (pixel->inBinPlaces + pixel->ouBinPlaces))
	         + (SL32)pixel->inPrecHaPix) / (SL32)pixel->inPrecPixel;
...
UB8 *altsignptr;
INTR altsignbyte=0,altsignbitsleft=0;
UB8 *localptr;
INTR localbyte=0,localbitsleft=0;
UB8 *ptr;
INTR item;
...
BOOLEAN gray = FC_ISGRAY(&if_state.fcCur);
...
if(LBmodel->italicSwitch)
  ...
  ml.standStemOU     = scale ((((SL32)ml.standardYstem[stanIndx])<<2), yPixel, gray);
  ml.standWidtOU    = scale ((SL32)TFgroup->standardYheight, yPixel, gray);  
  ml.FCWstanStemOU     = scale ((SL32)TFgroup->FCWstanYstem<<2, yPixel, gray);  
  ml.FCWstanWidtOU    = scale ((SL32)TFgroup->FCWstanYwidth, yPixel, gray);
...
#endif
    {
    ml.standStanOU   = scale (((SL32)stanStanStem<<2), yPixel, gray);
...
/* keb 6/1/06 */     
      ...
      if((SL32)ml.Ydimensions[dimIndex] == 255L) {
        dimension = scale (Yescapement, yPixel, gray);
	  }
	}
  
#if BOLD_FCO
    dimension += (yPixel->roundTyp[rt_indx]-yPixel->roundTyp[2]);
#endif
/* keb 6/1/06 */
#if USBOUNDBOX
	   /* keb fix 7/24/06 */
...
Yprevadj:
  ...
  parent = 0;
  parentrtyp = 1;
  ml.standStemOU     = scale ((((SW16)ml.standardXstem[stanIndx])<<2), xPixel, gray);
  ml.standWidtOU    = scale ((SL32)TFgroup->standardXwidth, xPixel, gray);  
  ml.FCWstanStemOU     = scale ((SL32)TFgroup->FCWstanXstem<<2, xPixel, gray);  
  ml.FCWstanWidtOU    = scale ((SL32)TFgroup->FCWstanXwidth, xPixel, gray);
...
/* keb 6/1/06 */
...
#endif
    {
    ml.standStanOU   = scale (((SL32)stanStanStem<<2), xPixel, gray);
  ...
  if(italicAngle == 0) {
    ...
    Xsyntell[1] = Xsyntell[0] + scale (Xescapement, xPixel, gray);
...
/* keb 6/1/06 */
        ...
        if((SL32)ml.Xdimensions[dimIndex] == 255L) {
          dimension = scale (Xescapement, xPixel, gray);
		} 
	  } 
  
#if BOLD_FCO
      dimension += (xPixel->roundTyp[rt_indx]-xPixel->roundTyp[2]);
#endif
/* keb 6/1/06 */
#if USBOUNDBOX
	      /* keb fix 7/24/06 */
...
if(localComp && localbitsleft == 8)
  localptr--;
*curveSegOff = (UW16)((ULPtr)localptr - (ULPtr)CHlocal);  /* setup curve data for putsegsII () */
